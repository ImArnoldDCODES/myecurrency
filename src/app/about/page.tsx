...
const About: React.FC<QuantitySelectorProps> = ({ initialQuantity = 1 }) => {
  const [quantity, setQuantity] = useState<number>(initialQuantity); // Initial quantity
  const pricePerUnit = 50; // Replace with the actual price per unit
  ...
  return (
    <main>
      <section className="bg-[url('/about_bg.svg')] bg-cover bg-origin-border lg:bg-right bg-top bg-no-repeat w-full h-screen flex">
          ...
          <div className="flex gap-10">
            ...
            <div>
              ...
              <input
                ...
                className="border-2 border-[#12305B] outline-none h-[1.5rem] w-[2.5rem] rounded w-[2rem]"
              />
            </div>
          </div>
